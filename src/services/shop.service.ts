...
const shopService = {
  createDefaultShopAddress: async (
    shopId: string,
    data: CreateDefaultShopAddressRequest,
  ) => {
  ...
  getShops: async (
    ...
    if (filters?.searchTerm) {
      ...
      query = query.or(
        `shop_name.ilike.${q},description.ilike.${q},shop_slug.ilike.${q},category.ilike.${q},state.ilike.${q},city.ilike.${q}`,
      );
    }
    ...
    // If searching, sort by relevance with weighted scoring
    ...
    if (filters?.searchTerm) {
      ...
      sortedShops = sortedShops
        .map((shop: any) => {
          ...
          const slug = (shop.shop_slug || "").toLowerCase();
    ...
    const shopIds = sortedShops.map((s: any) => s.id);
    let completeShops = sortedShops;
    ...
    if (shopIds.length > 0) {
      ...
      const shopsWithImages = new Set(
        productsData?.map((p: any) => p.shop_id) || [],
      );
    ...
    const mappedShops: Shop[] = completeShops.map((s: any) => ({
      ...
      tier:
        (s.safebeauty_tiers as any)?.[0]?.tier ||
        (s.safebeauty_tiers as any)?.tier ||
        "listed",
    }));
  ...
  getShopByOwner: async (ownerId: string) => {
    ...
    const mappedShops: Shop[] = (shops || []).map(s => ({
      ...
      seo_keywords: s.seo_keywords,
      seo_description: s.seo_description,
      seo_metadata: (s.seo_metadata as any) ?? undefined,
      seo_dna_updated_at: s.seo_dna_updated_at,
      created_at: s.created_at,
      tier:
        (s.safebeauty_tiers as any)?.[0]?.tier ||
  ...
  getShopBySlug: async (slug: string) => {
    ...
    const mappedShop: Shop = {
      ...
      seo_metadata: (shop.seo_metadata as any) ?? undefined,
      seo_dna_updated_at: shop.seo_dna_updated_at,
      created_at: shop.created_at,
      tier:
        (shop.safebeauty_tiers as any)?.[0]?.tier ||
        (shop.safebeauty_tiers as any)?.tier ||
        "listed",
    };
  ...
  updateShop: async (id: string, data: Partial<Shop>) => {
    ...
    if (data.slug || data.shop_slug)
      updateData.shop_slug = data.slug || data.shop_slug;
    if (data.description !== undefined)
      updateData.description = data.description;
    if (data.whatsapp || data.whatsapp_number)
    ...
    if (data.paystack_public_key)
      updateData.paystack_public_key = data.paystack_public_key;
    ...
    if (data.show_public_address !== undefined)
      updateData.show_public_address = data.show_public_address;
    if (data.seo_keywords !== undefined)
      updateData.seo_keywords = data.seo_keywords;
    if (data.seo_description !== undefined)
      updateData.seo_description = data.seo_description;
    if (data.seo_metadata !== undefined)
      updateData.seo_metadata = data.seo_metadata;
    if (data.seo_dna_updated_at !== undefined)
      updateData.seo_dna_updated_at = data.seo_dna_updated_at;
