...
const SlideSkeletons = () => (
  <>
    {[1, 2].map(i => (
      <div key={i} className="flex-shrink-0 w-full snap-start" aria-hidden>
...
export const FeaturedStoresHeroCarousel = () => {
  ...
  const [productIdxByShop, setProductIdxByShop] = useState<
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (featErr)
          console.warn("featured shops fetch error:", featErr.message);
        ...
        if (prodErr)
          console.warn("batch products fetch error:", prodErr.message);
        ...
        const cards: FeaturedShopCard[] = pool
          .map(f => {
            ...
            const tier = Array.isArray(tierData)
              ? tierData[0]?.tier
              : tierData?.tier || "listed";
          ...
          .filter(c => c.products.length > 0 && !!c.logo_url); // Must have at least one product with image AND a real logo
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isMobile
                      ? "flex-[0_0_90%] max-w-sm mx-auto"
                      : "flex-[0_0_min(100%,320px)] lg:flex-[0_0_320px]",
                  )}
                >
                  <Link
                    ...
                    <div className="rounded-xl overflow-hidden bg-card border border-border/50 shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-xl cursor-pointer">
                      ...
                      <div
                        className={cn(
                          ...
                          isMobile ? "p-3 pb-2.5" : "p-3.5 pb-3",
                        )}
                      >
                        {/* Logo */}
                        <div
                          className={cn(
                            "flex-shrink-0 overflow-hidden bg-muted/50 border border-border/50 flex items-center justify-center",
                            isMobile
