...
export const StoreFlyerTemplate = ({ shop, products = [] }: StoreFlyerTemplateProps) => {
        ...
        <head>
          ...
          <style>
            ...
            .flyer-container {
              ...
              ${backgroundImage ? `background-image: url(${backgroundImage}); background-size: ${backgroundSize};` : ''}
              margin: 0 auto;
              position: relative;
            }
            .print-element {
        ...
        <body>
          <div class="flyer-container">
            ${elements.map(element => {
              const style = `left:${element.x}px;top:${element.y}px;width:${element.width}px;height:${element.height}px;transform:rotate(${element.rotation}deg) scaleX(${element.flipX ? -1 : 1}) scaleY(${element.flipY ? -1 : 1});opacity:${element.opacity};`;
              
              switch (element.type) {
                case 'text':
                  return `<div class="print-element" style="${style}font-size:${element.fontSize}px;font-weight:${element.fontWeight};color:${element.color};text-align:${element.textAlign}">${element.content}</div>`;
                case 'image':
  ...
  const handleDownload = async () => {
    try {
      // For now, provide a simple text file with instructions
              ...
              <TabsContent value="design" className="space-y-4">
                <Card>
                  <CardContent className="p-4 space-y-3">
                    ...
                    <div className="space-y-2">
                      <Label>Upload Image to Canvas</Label>
                      <input
                        type="file"
                        ref={fileInputRef}
                        accept="image/*"
