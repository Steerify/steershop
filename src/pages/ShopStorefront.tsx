...
const normalizeStoredCart = (
  ...
  return parseStoredCart(storedValue).reduce<CartItem[]>(
    (items, storedItem) => {
      if (!storedItem || typeof storedItem !== "object") return items;

      const productId = (storedItem as { product?: { id?: unknown } }).product
        ?.id;
      ...
      items.push({
        ...
        quantity: clampCartQuantity(
          (storedItem as { quantity?: unknown }).quantity,
...
const ShopStorefront = () => {
  ...
  const [ownerPlan, setOwnerPlan] = useState<OwnerPlan>({
    slug: null,
    name: null,
  });
  ...
  const isPremiumPlan =
    ownerPlan.slug === "pro" || ownerPlan.slug === "business" || ownerIsInTrial;
  ...
  useEffect(() => {
    ...
    try {
      const savedCart = JSON.parse(
        localStorage.getItem(`cart_${shop.id}`) || "[]",
      );
  ...
  useEffect(() => {
    ...
    const hasCartOpenIntent =
      searchParams.get("cart") === "open" || location.hash === "#cart";
  ...
  const publicLocationParts = useMemo(
    () => [shop?.city, shop?.state, shop?.country].filter(Boolean).join(", "),
    [shop],
  );
  const fullPublicAddress = shop?.show_public_address
    ? [shop.address, shop.city, shop.state, shop.country]
        .filter(Boolean)
        .join(", ")
    : "";
  const shopCategoryLabel = shop?.category
    ? getCategoryLabel(shop.category) === "Other" && shop.category !== "other"
      ? shop.category
  ...
  }, [categoryFilter, searchQuery, products, typeFilter]);
  ...
  const quickCategoryChips = useMemo(() => {
    const chips = new Map<
      string,
      { key: string; label: string; imageUrl: string | null }
    >();

    products.forEach(product => {
      const rawKey = (product.category || product.type || "all")
        .trim()
        .toLowerCase();
      if (!rawKey || chips.has(rawKey)) return;
      ...
      const fallbackLabel = rawKey
        ...
        .join(" ");
      const mappedLabel = getCategoryLabel(rawKey);

      chips.set(rawKey, {
        key: rawKey,
        label: mappedLabel === "Other" ? fallbackLabel : mappedLabel,
    ...
    return [
      ...
      ...Array.from(chips.values()).slice(0, 7),
    ];
  }, [products, shop?.logo_url]);
  const activeCategoryLabel = useMemo(() => {
    if (categoryFilter === "all") return "Featured Products";
    return (
      quickCategoryChips.find(chip => chip.key === categoryFilter)?.label ||
      "Featured Products"
    );
  }, [categoryFilter, quickCategoryChips]);
  const storefrontHighlights = useMemo(
    () => [
      {
        icon: ShoppingBag,
        title: completedOrders > 0 ? "Completed Orders" : "Curated Catalog",
        subtitle:
          completedOrders > 0
            ? `${completedOrders} successful deliveries`
            : `${productCount + serviceCount} items ready to browse`,
      },
      {
        icon: BadgeCheck,
        title: shop?.is_verified ? "Verified Seller" : "Secure Storefront",
        subtitle: shop?.is_verified
          ? "Identity and trust checks passed"
          : "Protected by SteerSolo checkout",
      },
      {
        icon: MessageCircle,
        title: shop?.whatsapp_number ? "WhatsApp Support" : "SteerSolo Support",
        subtitle: shop?.whatsapp_number
          ? "Fast order conversations"
          : "Smooth checkout and support",
      },
    ],
    [
      completedOrders,
  ...
  const loadShopData = async () => {
    try {
      ...
      const transformedShop = {
        ...shopData,
        tier:
          (shopData.safebeauty_tiers as any)?.[0]?.tier ||
          (shopData.safebeauty_tiers as any)?.tier ||
          "listed",
      };
      ...
      if (shopData.owner_id) {
        const { data: profileData } = await supabase
          .from("profiles")
          .select(
            "subscription_plan_id, is_subscribed, subscription_expires_at",
          )
      ...
      const hydratedCart = normalizeStoredCart(
        localStorage.getItem(getCartKey(shopData.id)),
        productsList,
      );
  ...
  const updateAndPersistCart = (
    updater: (prevCart: CartItem[]) => CartItem[],
  ) => {
    setCart(prevCart => {
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full max-w-xs sm:max-w-none">
            <Link to="/shops">
              <Button
                size="lg"
                className="w-full rounded-xl px-8 bg-gradient-to-r from-accent to-primary font-semibold shadow-lg shadow-accent/25"
              >
  ...
  const isSetupIncomplete =
    !shop.payment_method ||
    (shop.payment_method === "bank_transfer" && !shop.bank_account_number) ||
    (shop.payment_method === "paystack" && !shop.paystack_public_key) ||
    (shop.payment_method === "both" &&
      (!shop.bank_account_number || !shop.paystack_public_key)) ||
  ...
  return (
    <div
      ...
      style={{
        ...(shop?.accent_color
          ? ({ "--accent": shop.accent_color } as any)
          : {}),
    ...
    >
      ...
      <Navbar
        shopBranding={
          isPremiumPlan
            ? { name: shop.shop_name, logoUrl: shop.logo_url }
            : null
        }
      />
            ...
            <div className="container mx-auto px-4 flex items-center justify-between gap-4">
              <div className="flex items-center gap-2 text-amber-800 dark:text-amber-400 text-sm font-medium">
                ...
                <span>
                  ...
                  payment details to enable checkout.
                </span>
              </div>
              <Link to="/dashboard">
                <Button
