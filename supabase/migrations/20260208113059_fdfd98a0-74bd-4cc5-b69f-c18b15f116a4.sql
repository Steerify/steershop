...
-- Add location columns to shops
ALTER TABLE public.shops ADD COLUMN IF NOT EXISTS country text DEFAULT 'Nigeria';
ALTER TABLE public.shops ADD COLUMN IF NOT EXISTS state text;

-- Add delivery_method to onboarding_responses
ALTER TABLE public.onboarding_responses ADD COLUMN IF NOT EXISTS delivery_method text;

-- Update handle_new_user trigger to fallback to email prefix
...
BEGIN
  ...
  -- Get full_name, fallback to email prefix
