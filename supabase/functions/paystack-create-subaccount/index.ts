...
serve(async req => {
  ...
  try {
    ...
    const { shop_id, business_name, bank_code, account_number, verify_only } =
      body;
    ...
    // Verify shop ownership if not just verifying
    if (!verify_only) {
      if (!shop_id || !business_name) {
        return new Response(
          JSON.stringify({ error: "Missing required fields" }),
          {
            status: 400,
    ...
    // If only verifying, return early
    if (verify_only) {
      return new Response(
        JSON.stringify({
          success: true,
    ...
    // Get bank name from the banks list (using the bank code)
