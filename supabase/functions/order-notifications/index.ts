...
serve(async (req) => {
  ...
  try {
    ...
    const { orderId, eventType, shopName, customerEmail, customerName, totalAmount, items, statusUpdate } = await req.json();
    ...
    switch (eventType) {
      case "order_placed":
        subject = `Order Confirmed #${shortId} - ${shopName}`;
        bodyHtml = `
      ...
      case "status_update":
        ...
        subject = `Order #${shortId} - ${statusLabel.charAt(0).toUpperCase() + statusLabel.slice(1)}`;
