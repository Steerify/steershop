...
// The user requested to ignore the strict time schedule and just pick a random slot
function pickRandomSlotForGroup(group: TargetGroup): Slot {
  ...
  try {
    ...
    await Promise.all(targetGroups.map(async (group) => {
      try {
        ...
        const slot = forcedSlot ?? pickRandomSlotForGroup(group);
        if (!slot) return;
